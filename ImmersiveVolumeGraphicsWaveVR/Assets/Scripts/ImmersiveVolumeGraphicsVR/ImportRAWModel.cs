...
namespace UnityVolumeRendering
{
    ...
    public class ImportRAWModel : MonoBehaviour
    {
        ...
        public void OpenRAWData()
        {
            ...
            // Parse .ini file
            initData = DatasetIniReader.ParseIniFile(Application.persistentDataPath+"/" + ModelPath + ".ini");
            if (initData != null)
            {
                // Import the dataset
                RawDatasetImporter importer = new RawDatasetImporter(Application.persistentDataPath+"/"+ ModelPath + ".raw", initData.dimX, initData.dimY, initData.dimZ, initData.format, initData.endianness, initData.bytesToSkip);
                ...
                if (dataset != null)
                {
                    ...
                    if (DICOMMetaReader.getThickness() > 0)
                    {
                        ...
                        volobj.gameObject.transform.localScale = new Vector3((initData.dimX * 1.0f*0.5f) / 1000, (initData.dimY * 1.0f * 0.5f) / 1000, (initData.dimZ * 1.0f * 0.5f) / 1000);
                    }

                    volobj.gameObject.transform.localScale = new Vector3(0.3f,0.3f,0.3f);
